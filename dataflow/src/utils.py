...
def upload_local_file_to_gcp_storage_bucket(storage_client, bucket,folder,blob_name, data_file):
    BUCKET = storage_client.get_bucket(bucket)
...
def create_load_bigquery_table(bigquery_client, dataset, table, gcs_source_file):    
    ...
    bigqueryJob = bigquery_client.load_table_from_uri(gcs_source_file, tableRef, job_config=jobConfig)
    bigqueryJob.result()